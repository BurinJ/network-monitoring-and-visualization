...
def get_metric_history(query, hours=24, step='1h'):
    ...
    try:
        ...
        # custom_query_range returns a list of metrics with a 'values' list
        ...
        if result and len(result) > 0:
            # result[0]['values'] is a list of [timestamp, value]
    ...
    try:
        ...
        for item in results:
            ...
            lan_probes.append({
                ...
                "dns": round(lan_dns, 2),
                "lat": lat, # Pass coordinates
                "lng": lng
            })

            # WLAN
            # REMOVED: wlan_ipv4 == "None" check based on requirement
...
def fetch_command_center():
    ...
    for p in all_interfaces:
        ...
        # Use name/id as key to group LAN/WLAN (assume same name = same location)
        key = p['name']
        ...
        # If ANY interface for this location is red, mark the map pin red
        if p['color'] == 'red':
    ...
    return {
        ...
        "map_markers": list(map_markers.values()) # <--- Added this back
    }

# --- PAGE 3: INSPECTOR LOGIC (UPDATED) ---
def fetch_inspector_data(probe_id):
    clean_id = probe_id
    ...
    # --- FETCH HISTORY FOR GRAPH (UPDATED WITH SPLIT) ---
    # LAN History
    lan_hist_ext_down = get_metric_history(f'LAN_EXTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Download"}}')
    lan_hist_ext_up   = get_metric_history(f'LAN_EXTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Upload"}}')
    lan_hist_int_down = get_metric_history(f'LAN_INTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Download"}}')
    lan_hist_int_up   = get_metric_history(f'LAN_INTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Upload"}}')
    ...
    if has_wlan:
        wlan_hist_ext_down = get_metric_history(f'WLAN_EXTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Download"}}')
        wlan_hist_ext_up   = get_metric_history(f'WLAN_EXTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Upload"}}')
        wlan_hist_int_down = get_metric_history(f'WLAN_INTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Download"}}')
        wlan_hist_int_up   = get_metric_history(f'WLAN_INTERNAL_SPEEDTEST{{hostname="{raw_hostname}", type="Upload"}}')
    ...
    diagnosis = {"status": "Healthy", "title": "Normal Operation", "desc": "No significant anomalies detected."}
    
    if is_stale:
         diagnosis = {"status": "Critical", "title": "Probe Offline", "desc": f"Probe unresponsive for > 1 hour. Last seen: {stale_label}"}
    elif lan_ping == 0 and (not has_wlan or wlan_ping == 0):
         diagnosis = {"status": "Critical", "title": "Probe Offline", "desc": "Interfaces unresponsive."}
    elif error_label == "CURL&DNS&WLAN-ERR":
         diagnosis = {"status": "Critical", "title": "WLAN Failure", "desc": "Critical failure in Wi-Fi interface (IP/DNS/Curl)."}
    elif has_wlan and wlan_dns > 500:
         diagnosis = {"status": "Critical", "title": "WLAN DNS Failure", "desc": "Wi-Fi cannot resolve domain names."}
    elif lan_dns > 500:
         diagnosis = {"status": "Critical", "title": "LAN DNS Failure", "desc": "Ethernet DNS resolution failed."}
    elif has_wlan and wlan_ping > 200:
         diagnosis = {"status": "Warning", "title": "Wi-Fi Congestion", "desc": "High latency on Wi-Fi interface."}

    return {"metrics": data, "ai_diagnosis": diagnosis, "has_wlan": has_wlan}
