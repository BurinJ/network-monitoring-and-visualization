...
const TrendGraph = ({ 
  ...
  function getAvgY(val) { return val ? height - ((val - minVal) / range) * height : 0; }
            ...
            {showDown && (
              <>
                ...
                {historyDown.map((d, i) => {
                  ...
                  const y = height - ((d[1] - minVal) / range) * height;
                  return (
                    <circle key={`d-${i}`} cx={x} cy={y} r="3" fill="#10b981" className="hover:r-5 transition-all cursor-pointer opacity-0 hover:opacity-100">
                      <title>{`${formatTime(d[0])} - ${historyUp ? `${tooltipDown}: ` : ''}${Math.round(d[1])} ${unit}`}</title>
                    </circle>
                  );
                })}
                {avgDown > 0 && <line x1="0" y1={yAvgDown} x2="300" y2={yAvgDown} stroke="#10b981" strokeWidth="1" strokeDasharray="4,2" opacity="0.6" vectorEffect="non-scaling-stroke" />}
              </>
            )}
            ...
            {showUp && historyUp && (
              <>
                ...
                {historyUp.map((d, i) => {
                  ...
                  const y = height - ((d[1] - minVal) / range) * height;
                  return (
                    <circle key={`u-${i}`} cx={x} cy={y} r="3" fill="#3b82f6" className="hover:r-5 transition-all cursor-pointer opacity-0 hover:opacity-100">
                      <title>{`${formatTime(d[0])} - ${tooltipUp}: ${Math.round(d[1])} ${unit}`}</title>
                    </circle>
                  );
                })}
                {avgUp > 0 && <line x1="0" y1={yAvgUp} x2="300" y2={yAvgUp} stroke="#3b82f6" strokeWidth="1" strokeDasharray="4,2" opacity="0.6" vectorEffect="non-scaling-stroke" />}
              </>
            )}
          </svg>
        </div>
...
// --- Diagnosis Card Component with Dropdown ---
  ...
  const SpeedSection = ({ title, icon: TitleIcon, external, internal, history, average, color, speedCap }) => (
    <div className="bg-white border border-teal-100 rounded-lg p-4 shadow-sm">
      ...
      <div className="mb-6">
        ...
        <TrendGraph title={`External Trend (${timeRange})`} historyDown={history.external.down} historyUp={history.external.up} avgDown={average.external.down} avgUp={average.external.up} color={color} height={120} />
      </div>
      ...
      <div className="pt-4 border-t border-dashed border-teal-50">
        ...
        <TrendGraph title={`Internal Trend (${timeRange})`} historyDown={history.internal.down} historyUp={history.internal.up} avgDown={average.internal.down} avgUp={average.internal.up} color={color} height={120} />
      </div>
    </div>
  );
  ...
  const LatencySection = ({ history, average, color }) => (
    <div className="bg-white border border-teal-100 rounded-lg p-4 shadow-sm">
      ...
      <TrendGraph 
        ...
        color={color} height={100} unit="ms" labelDown="Ext" labelUp="Int" tooltipDown="External Ping" tooltipUp="Internal Ping"
      />
    </div>
  );
    ...
    <div className="p-6 bg-teal-50 min-h-screen">
      ...
      <div className={`grid grid-cols-1 ${showWlan ? 'lg:grid-cols-2' : ''} gap-8`}>
        <div className="bg-teal-100/50 p-6 rounded-2xl border border-teal-200 flex flex-col gap-6">
          ...
          <LatencySection history={metrics.lan.history.ping} average={metrics.lan.average.ping} color="emerald" />
          <SpeedSection title="Wired" icon={Network} external={metrics.lan.speed.external} internal={metrics.lan.speed.internal} history={metrics.lan.history} average={metrics.lan.average} color="emerald" speedCap={metrics.lan.speed_cap} />
        </div>
        ...
        {showWlan && (
          <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100 flex flex-col gap-6">
            ...
            <LatencySection history={metrics.wlan.history.ping} average={metrics.wlan.average.ping} color="purple" />
            <SpeedSection title="Wireless" icon={Globe} external={metrics.wlan.speed.external} internal={metrics.wlan.speed.internal} history={metrics.wlan.history} average={metrics.wlan.average} color="purple" speedCap={metrics.wlan.speed_cap} />
          </div>
        )}
      </div>
    </div>
